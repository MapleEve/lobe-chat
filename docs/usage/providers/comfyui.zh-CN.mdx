...
<Steps>
  ...
  **启动 ComfyUI（无认证）**：
  ...
  # Docker 方式
  docker run -p 8000:8000 comfyui/comfyui
  ```
  ...
  **启动配置**：
  ...
  python main.py --listen 0.0.0.0 --port 8000 --auth-token
  ```
  ...
  **验证 Token 有效性**：
  ...
  curl -H "Authorization: Bearer $COMFYUI_TOKEN" \
       http://your-server:8000/system_stats
  ...
  **启动 ComfyUI 使用自定义认证**：
  ...
  python main.py --listen 0.0.0.0 --port 8000 --custom-auth
  ```
